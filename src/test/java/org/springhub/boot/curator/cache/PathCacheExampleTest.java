...
@SpringBootTest
public class PathCacheExampleTest {
    ...
    private void addListener(CuratorCache cache) {
        // a PathChildrenCacheListener is optional. Here, it's used just to log
        // changes
        CuratorCacheListener listener = (type, oldData, newData) -> {
            switch (type) {
                case NODE_CREATED: {
                    System.out.println("Node added: " + ZKPaths.getNodeFromPath(newData.getPath()) + ", value: "
                            + new String(newData.getData(), StandardCharsets.UTF_8));
                    break;
                }
                case NODE_CHANGED: {
                    System.out.println("Node changed: " + ZKPaths.getNodeFromPath(newData.getPath()) + ", value: "
                            + new String(newData.getData(), StandardCharsets.UTF_8));
                    break;
                }
                case NODE_DELETED: {
                    System.out.println("Node removed: " + ZKPaths.getNodeFromPath(oldData.getPath()));
                    break;
                }
                default:
                    break;
    ...
    private void processCommands(CuratorCache cache) throws Exception {
        ...
        String[] commands = {"set kk kk缓存的值", "set aa aa缓存的值", "set bb bb缓存的值", "list", "remove kk", "remove aa", "remove bb"};
