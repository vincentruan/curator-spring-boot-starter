...
public class DiscoveryExampleTest {
    ...
    @Test
    public void testDiscoveryExample() throws Exception {
		...
		try {
            ...
            providers.values().forEach(CloseableUtils::closeQuietly);
	...
	private static void processCommands(ServiceDiscovery<InstanceDetails> serviceDiscovery, Map<String, ServiceProvider<InstanceDetails>> providers,
		...
		try {
            ...
            String[] commands = {"add server1 服务节点1", "add server2 服务节点2", "add server3 服务节点3", "list", "random server3", "random server2", "delete server2"};
